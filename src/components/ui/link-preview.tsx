...
export const HoverPeek = ({
    ...
    // Defines how the main preview card animates in and out.
    // A gentle fade + scale (no 3D flip) so it feels smooth whether Radix places
    // it above or, when there's no room, collapses it below the trigger.
    const cardMotionVariants: Variants = {
        initial: { opacity: 0, scale: 0.92 },
        animate: { opacity: 1, scale: 1, transition: { duration: 0.24, ease: [0.22, 1, 0.36, 1] } },
        exit: { opacity: 0, scale: 0.96, transition: { duration: 0.16, ease: [0.22, 1, 0.36, 1] } },
    };
